...
function buildState(s){
    ...
    for(let i=0; i<total; i++){
      ...
      const t = {
        ...
        ...(() => {
          ...
          /* ALL desktops are "installed" on the terminal — visibility only controls default boot */
          const installedDts = allDts.filter(d=>{
            if(useLbl!=='教师终端' && /教师/.test(d.name)) return false;
            return true;
          });
          const termDts = useLbl==='教师终端' ? [...allDts] : installedDts;
          const bootableDts = termDts.filter(d=>d.visibility!=='hidden' && !d.physicalDeploy);
          const validDefault = bootableDts.find(d=>d.id===sc.defaultDesktopId);
          const effectiveDefault = validDefault ? sc.defaultDesktopId : (bootableDts[0]?.id||null);
          ...
          return {
            desktops: termDts.map(d=>enrichDt(d)),
            defaultDesktopId: isBlank ? null : effectiveDefault,
            bios: isBlank ? null : {
              bootEntries: bootableDts.map(d=>d.id),
  ...
  return {
    ...
    demo: {
      ...
      deployDraft: {
        step: 0, /* 0=motherPrep, 1=scopeRules, 2=desktopSelect, 3=confirm, 4=executing */
        motherPrep: { name:null, ip:null, seat:null, use:null, gateway:null, dns:null, subnetMask:'255.255.255.0', serverAddr:null, desktopReady:false },
        scope: [], /* terminal IDs in order */
        rules: {
          ipBase: '', ipStart: 20,
          namePrefix: '', nameStart: 1,
          seatSeatsPerCol: 7, seatStartCol: 'A',
          defaultUse: '学生终端', serverAddress: '',
          gateway: '', dns: '', subnetMask: '255.255.255.0',
          restoreMode: '还原系统盘，保留数据盘',
          contentMode: '整体覆盖现有桌面'
        },
        desktopIds: [], defaultDesktopId: null, dataDisk: '新建 20G VHD',
        assignments: [], /* {terminalId, name, seat, ip, use, ...} */
      ...
      examDraft: { step:0, scope:[], keepIds:[], desktopId:null, desktopIds:[], defaultDesktopId:null, hideEntries:true, restoreMode:'还原系统盘和数据盘' },
...
function act(action, payload={}){
  ...
  case 'switch-focus':{
    ...
    demo.deployDraft.rules.ipBase=newCr.networkBase; demo.deployDraft.rules.serverAddress=newCr.serverAddress||'';
    demo.deployDraft.rules.namePrefix=payload.classroomId.split('-')[1].toUpperCase();
    demo.deployDraft.rules.gateway=newCr.gateway||''; demo.deployDraft.rules.dns=((newCr.dns||[]).join(','))||'';
    demo.deployDraft.rules.seatSeatsPerCol=newCr.rows||7;
    ...
    demo.examDraft={step:0,scope:[],keepIds:[],desktopId:null,desktopIds:[],defaultDesktopId:null,hideEntries:true,restoreMode:'还原系统盘和数据盘'};
  ...
  case 'open-fault': demo.motherScreen='fault-menu'; mt.screen='fault-menu'; return {ok:true};
  ...
  case 'confirm-takeover':{
    ...
    demo.deployDraft.rules.serverAddress=cr.serverAddress||'';
    demo.deployDraft.rules.gateway=cr.gateway||''; demo.deployDraft.rules.dns=((cr.dns||[]).join(','))||'';
    demo.deployDraft.rules.seatSeatsPerCol=cr.rows||7;
  ...
  case 'open-deployment':{
    if(mt.controlState!=='mother') return {ok:false,reason:'未接管教室'};
    const prep=demo.deployDraft.motherPrep;
    prep.name=mt.name; prep.ip=mt.ip; prep.seat=mt.seat; prep.use=mt.use||'教师终端';
    prep.gateway=mt.gateway||cr.gateway; prep.dns=(mt.dns||cr.dns||[]).join(',');
    prep.subnetMask=mt.subnetMask||'255.255.255.0';
    prep.serverAddr=mt.serverAddr||cr.serverAddress||'';
    prep.desktopReady=mt.desktops.length>0;
    // auto-select all controlled online default-use terminals (exclude teacher etc.)
    const allCtrl = termsInCr(cr.id).filter(t=>t.id!==mt.id&&t.online);
    demo.deployDraft.scope=allCtrl.filter(t=>!t.use||t.use==='学生终端').map(t=>t.id);
    // init rules defaults from classroom
    demo.deployDraft.rules.ipBase=demo.deployDraft.rules.ipBase||cr.networkBase;
    demo.deployDraft.rules.namePrefix=demo.deployDraft.rules.namePrefix||(cr.id.split('-')[1].toUpperCase());
    demo.deployDraft.rules.serverAddress=demo.deployDraft.rules.serverAddress||cr.serverAddress||'';
    demo.deployDraft.rules.gateway=demo.deployDraft.rules.gateway||cr.gateway||'';
    demo.deployDraft.rules.dns=demo.deployDraft.rules.dns||((cr.dns||[]).join(','))||'';
    demo.deployDraft.rules.seatSeatsPerCol=demo.deployDraft.rules.seatSeatsPerCol||cr.rows||7;
    // For re-deploy: auto-skip to step 1 if mother is already configured
    const motherReady = mt.name && mt.ip && mt.desktops.length>0 && cr.stage==='deployed';
    if(motherReady){
      demo.deployDraft.step=1;
      demo.motherScreen='deploy-main'; mt.screen='deploy-main';
    } else {
      demo.deployDraft.step=0;
      demo.motherScreen='deploy-mother-prep'; mt.screen='deploy-mother-prep';
    }
    return {ok:true};
  }
  case 'deploy-save-mother-prep':{
    const p=payload;
    if(p.name!==undefined){ demo.deployDraft.motherPrep.name=p.name; mt.name=p.name; }
    if(p.seat!==undefined){ demo.deployDraft.motherPrep.seat=p.seat; mt.seat=p.seat; }
    if(p.use!==undefined){ demo.deployDraft.motherPrep.use=p.use; mt.use=p.use; }
    if(p.ip!==undefined){ demo.deployDraft.motherPrep.ip=p.ip; mt.ip=p.ip; }
    if(p.gateway!==undefined){ demo.deployDraft.motherPrep.gateway=p.gateway; mt.gateway=p.gateway; }
    if(p.dns!==undefined){ demo.deployDraft.motherPrep.dns=p.dns; mt.dns=p.dns.split?p.dns.split(','):p.dns; }
    if(p.subnetMask!==undefined){ demo.deployDraft.motherPrep.subnetMask=p.subnetMask; mt.subnetMask=p.subnetMask; }
    if(p.serverAddr!==undefined){ demo.deployDraft.motherPrep.serverAddr=p.serverAddr; mt.serverAddr=p.serverAddr; }
    return {ok:true};
  }
  ...
  case 'deploy-goto-step':{
    ...
    if(step===2){ /* going to desktop select */
      demo.deployDraft.assignments=generateAssignments(demo,cr);
    }
    if(step===3){ /* going to confirm */
      demo.deployDraft.assignments=generateAssignments(demo,cr);
      demo.deployDraft.validation=validateAssignments(demo.deployDraft.assignments);
    }
    demo.deployDraft.step=step;
    demo.motherScreen=step===0?'deploy-mother-prep':step===1?'deploy-main':step===2?'deploy-desktop':step===3?'deploy-confirm':'deploy-progress';
  ...
  case 'start-deployment':{
    const scope=demo.deployDraft.scope.filter(id=>{const t=termById(id);return t&&t.online;});
    if(!scope.length) return {ok:false,reason:'无有效终端'};
    const assignments=demo.deployDraft.assignments;
    const primaryDtId=demo.deployDraft.defaultDesktopId||cr.defaultDesktopId;
    const task=createTask({type:'deployment',crId:cr.id,label:'单教室部署',sourceDesktopId:primaryDtId,
      selectedIds:scope, settings:{rules:{...demo.deployDraft.rules}}, assignments});
    startTask(task);
    demo.deployDraft.step=4;
    demo.motherScreen='deploy-progress'; mt.screen='deploy-progress';
    addLog('info','终端','开始单教室部署: '+cr.name, scope.length+' 台终端');
    return {ok:true};
  }
  ...
  case 'open-maint-menu':{
    ...
    const allCtrl=termsInCr(cr.id).filter(t=>t.id!==mt.id&&t.online);
    demo.maintDraft.scope=allCtrl.map(t=>t.id);
    demo.motherScreen='maint-menu'; mt.screen='maint-menu'; return {ok:true};
  }
  ...
  case 'maint-goto-step':{
    ...
    const sMap={0:'maint-menu',1:'maint-desktop-update',select:'maint-desktop-select',2:'maint-confirm',3:'maint-progress'};
    demo.motherScreen=sMap[payload.step]||'maint-menu'; mt.screen=demo.motherScreen;
    return {ok:true};
  }
  ...
  case 'exam-set-desktops':{
    ...
    if(payload.defaultDesktopId!==undefined) demo.examDraft.defaultDesktopId=payload.defaultDesktopId;
    return {ok:true};
  }
  ...
  case 'return-from-desktop':{
    ...
    demo.deployDraft.motherPrep.desktopReady=true;
    demo.motherScreen='desktop-merging'; mt.screen='desktop-merging';
  ...
  case 'fault-replace-back':{
    ...
    delete demo.faultReplace.forceStep;
  ...
  case 'fault-replace-goto-0':{
    // Go back to step 0 (preconditions) — allow user to review, keep serverReachable intact
    demo.faultReplace.forceStep=0;
    demo.faultReplace.suggestedTerminalId=null;
    demo.faultReplace.suggestedSeat=null;
    demo.faultReplace.suggestedName=null;
    demo.faultReplace.suggestedMac=null;
  ...
  case 'fault-replace-goto-1':{
    // Proceed to step 1 (select classroom+terminal) — clear forceStep
    delete demo.faultReplace.forceStep;
    return {ok:true};
  }
  ...
  case 'advance-story':{
    ...
    if(cr.stage==='blank'||cr.stage==='bound'){
      ...
      if(demo.motherScreen==='deploy-mother-prep'){
        return act('deploy-goto-step',{step:1});
      }
      if(demo.motherScreen==='deploy-main'){
        if(demo.deployDraft.step<2) return act('deploy-goto-step',{step:2});
        return act('deploy-goto-step',{step:2});
      }
      if(demo.motherScreen==='deploy-desktop'){
        if(mt.desktops.length===0) return act('enter-desktop');
        if(!demo.deployDraft.desktopIds.length){
          demo.deployDraft.desktopIds=mt.desktops.map(d=>d.id);
          demo.deployDraft.defaultDesktopId=mt.defaultDesktopId||mt.desktops[0]?.id;
        }
        return act('deploy-goto-step',{step:3});
      }
      ...
      if(demo.motherScreen==='deploy-confirm') return act('start-deployment');
      return {ok:true};
    }
    /* deployed classroom */
    if(demo.motherScreen==='workbench') return act('open-maint-menu');
    if(demo.motherScreen==='maint-menu'){
      return act('open-maint-desktop-update');
    }
    if(demo.motherScreen==='maint-desktop-update'){
      if(!demo.maintDraft.desktopId&&mt.desktops.length) demo.maintDraft.desktopId=mt.desktops[0].id;
      return act('maint-goto-step',{step:'select'});
    }
    if(demo.motherScreen==='maint-desktop-select') return act('start-maintenance');
    if(demo.motherScreen==='maint-desktop') return act('maint-goto-step',{step:2});
    if(demo.motherScreen==='maint-confirm') return act('start-maintenance');
    return {ok:true};
  }
  ...
  case 'plat-remote-test':{
    ...
    const results=terms.map(t=>{
      const hasIssue=Math.random()<0.05; // 5% chance of issue
      return {
        id:t.id, seat:t.seat||t.mac, name:t.name||'', ip:t.ip||'',
        latency:hasIssue?(Math.floor(Math.random()*200+50)+'ms'):(Math.floor(Math.random()*8+1)+'ms'),
        bandwidth:hasIssue?(Math.floor(Math.random()*50+10)+'Mbps'):(Math.floor(Math.random()*400+600)+'Mbps'),
        upBandwidth:hasIssue?(Math.floor(Math.random()*20+5)+'Mbps'):(Math.floor(Math.random()*200+300)+'Mbps'),
        serverReachable:!hasIssue||Math.random()>0.5, gatewayReachable:!hasIssue||Math.random()>0.3
      };
    });
    addLog('info','平台','终端网络连通性测试',`已测试 ${results.length} 台终端`);
    return {ok:true, results};
  }
  case 'plat-broadcast-test':{
    const crId=payload.classroomId; const peerIds=payload.classroomIds;
    if(!crId||!peerIds||!peerIds.length) return {ok:false,reason:'请选择至少一个对端教室'};
    ...
    const results=peerIds.map(peerId=>{
      const peerCr=crById(peerId); const peerTerms=termsInCr(peerId).filter(t=>t.online);
      const peerPick=peerTerms[Math.floor(Math.random()*peerTerms.length)];
      const leaked=Math.random()<0.15; // 15% chance of leak
      const localForeign=leaked?Math.floor(Math.random()*8)+2:0;
      const peerForeign=leaked?Math.floor(Math.random()*8)+2:0; // bidirectional
      return {
        peerClassroom:peerCr?.name||peerId,
        leaked,
        localSeat:localPick?.seat||'--', localIp:localPick?.ip||'--',
        localSent:100, localRecvForeign:localForeign,
        peerSeat:peerPick?.seat||'--', peerIp:peerPick?.ip||'--',
        peerSent:100, peerRecvForeign:peerForeign
      };
    });
    const hasInterference=results.some(r=>r.leaked);
    addLog(hasInterference?'warn':'info','平台','教室广播隔离测试',
  ...
  case 'plat-import-terminal-list':{
    ...
    if(cr){
      if(payload.crName) cr.name=payload.crName;
      if(payload.crRemark) cr.remark=payload.crRemark;
      if(!cr.registeredOnServer){
        cr.registeredOnServer=true;cr.lastSyncTime=now();
      }
      const count=termsInCr(crId).length;
      addLog('info','平台','终端清单导入',`${cr.name} 已完成服务器注册建档（${count} 台终端）`);
...
async function main(){
  if(existsSync(STATE_FILE)){
    try{
      ...
      state = (loaded && loaded.classrooms) ? loaded : buildState(seed);
    }catch(e){ state=buildState(seed); }
