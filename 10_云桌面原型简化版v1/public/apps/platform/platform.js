...
function breadcrumb(){
  ...
  if(view.page==='classrooms'){
    if(view.classroomId){
      ...
      if(view.terminalId){ const t=getTerm(s(),view.terminalId); parts.push(`<span>${esc(t?.name||t?.mac||'')}</span>`); }
    }
  }
...
function crTerminalsTab(c,terms){
  ...
  <table class="data-table">
    <thead><tr><th style="width:32px"><input type="checkbox" data-sel-toggle-all${sel.length===terms.length&&terms.length?' checked':''}></th><th>座位号</th><th>机器名</th><th>用途</th><th>IP</th><th>工作 MAC</th><th>在线</th></tr></thead>
    <tbody>${terms.map((t,i)=>{
...
function crLayoutInline(c,terms){
  ...
  function cellHtml(t){
    ...
    return `<div class="${cls}" ${isRunning?'':`data-term-sel="${t.id}"`} title="${esc(t.seat||'')} · ${esc(t.name||'')} · ${esc(t.ip||'无IP')} · MAC:${esc(t.mac)}${t.online?' · 在线':' · 离线'}" style="width:${cellW}px;height:${cellH}px;font-size:${fontSize}rem;display:flex;flex-direction:column;justify-content:center;position:relative;overflow:hidden">
...
function platActionPanel(pa, par, c, terms){
  ...
  function termPills(list, max){
    ...
    return show.map(t=>`<span style="display:inline-block;padding:1px 6px;border-radius:3px;font-size:.75rem;background:var(--c-bg2);margin:1px">${esc(t.seat||t.mac.slice(-8))}</span>`).join('')
      ...
      <div style="display:grid;grid-template-columns:${srcDesktops.length?'280px 1fr':'1fr'};gap:16px;align-items:start">
        <div>
          ...
          <select data-dist-src style="width:100%;padding:6px 8px;border:1px solid var(--c-border);border-radius:4px">
            ...
            ${sourceCandidates.map(t=>`<option value="${t.id}"${t.id===selSrc?' selected':''}>${esc(t.seat||t.mac)} · ${esc(t.name||'')} · ${(t.desktops||[]).length} 个桌面</option>`).join('')}
          </select>
          ${srcTerm?`<div style="font-size:.78rem;color:var(--c-text3);margin-top:4px">IP: ${esc(srcTerm.ip||'--')} · MAC: ${esc(srcTerm.mac)}</div>`:''}
        </div>
    ...
    <div style="padding:12px 20px">
      ...
      <div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--c-border);font-size:.82rem">
        ...
        ${isExpanded&&tUsing.length?`<div style="margin-top:6px;margin-left:8px">${tUsing.map(t=>`<div style="display:flex;align-items:center;gap:8px;padding:2px 0">
          <a class="clickable" data-nav-term="${t.id}" style="cursor:pointer;text-decoration:underline;color:var(--c-brand)">${esc(t.seat||t.mac.slice(-5))}</a>
  ...
  return `<div class="alert-row ${a.level}">
    <div><div class="al-title">${pill(levelLabel[a.level]||a.level,a.level==='high'?'err':a.level==='medium'?'warn':'muted')} ${esc(a.title)}</div>
      ...
      <div class="al-source">
        ...
        ${t?`<a class="clickable" data-nav-term="${t.id}" style="cursor:pointer;text-decoration:underline;color:var(--c-brand)">${pill(t.seat||t.mac,'muted')} ${esc(t.name||'')}</a> ${t.ip?`<span class="mono">${esc(t.ip)}</span>`:''}`:''}
      </div></div>
...
function terminalDetailPage(){
  ...
  <div style="display:flex;align-items:baseline;gap:12px;margin-bottom:16px;flex-wrap:wrap">
    ...
    <h2 style="font-size:1.2rem">${esc(t.name||t.mac)}</h2>
  ...
  <div class="detail-grid">
    ...
    <div class="card">
      ...
      ${(()=>{
        const macs = t.macList||[];
        return macs.map((mac,idx)=>`<div class="def-row"><span class="def-label">${idx===0?'网卡':'备用网卡'}</span><span class="def-value mono">${esc(mac)}${mac===t.mac?' '+pill('工作中','ok'):''}</span></div>`).join('');
      })()}
      ${defRow('处理器', t.hw?.cpu||'--')}
      ${defRow('显卡', t.hw?.gpu||'--')}
      ${defRow('内存', t.hw?.mem||'--')}
