...
function homeScreen(){
  ...
  <div style="display:flex;flex-direction:column;gap:12px;width:320px">
    <button class="btn btn-secondary" style="padding:14px;font-size:1rem;justify-content:center" data-act="open-local-info">本机配置</button>
    <button class="btn btn-secondary" style="padding:14px;font-size:1rem;justify-content:center" data-act="open-local-network">服务器连接</button>
    <button class="btn btn-secondary" style="padding:14px;font-size:1rem;justify-content:center" data-act="open-local-desktop">桌面管理</button>
    ...
    <button class="btn btn-secondary" style="padding:14px;font-size:1rem;justify-content:center" data-act="open-fault-reset-direct">一键重置</button>
    <div style="border-top:1px solid var(--t-border);padding-top:16px;margin-top:4px">
      <button class="btn btn-primary" style="padding:14px;font-size:1rem;width:100%;justify-content:center" data-act="open-takeover">${m.controlState==='mother'?'已接管教室 — 进入工作台':'教室发送'}</button>
    </div>
  </div>
</div>`;
}
...
function localDesktopScreen(){
  ...
  ${desktops.length ? '<div class="page-scroll">' + desktops.map(d=>{
    ...
    return `
    <div class="dt-card mb-8 ${isDefault?'selected':''}" data-dt-id="${d.id}" style="position:relative;overflow:hidden">
      ...
      ${d.remark?`<div class="dt-sw">备注: ${esc(d.remark)}</div>`:''}      ${dataDisks.length?`<div class="dt-sw">数据盘: ${dataDisks.map(dd=>esc(dd.drive||'D:')+' '+esc(dd.size||'20GB')+(dd.sharedWith?.length?' '+pill('共享','info'):'')).join(', ')}</div>`:''}
      <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;align-items:center">
        ...
        ${!uploaded?`<button class="btn btn-primary btn-sm" data-dt-upload="${d.id}">上传</button>`:''}
      </div>
      </div>
    </div>`;
...
function takeoverScreen(){
  ...
  <div class="section-sub">终端扫描结果（仅在线已响应终端）· ${terms.length} 台</div>
  <div class="page-scroll">
  <div style="display:flex;flex-wrap:wrap;gap:6px">
    ${terms.map((t,i)=>`<div style="display:inline-flex;align-items:center;gap:6px;padding:4px 10px;background:var(--t-panel);border:1px solid var(--t-border);border-radius:4px;font-size:.8rem">
      <span style="color:var(--t-text3);min-width:20px">${i+1}</span>
...
/* ═══════════════════════════════════════════════════
   WORKBENCH — updated: no exam, rename deploy → 教室维护
   ═══════════════════════════════════════════════════ */
function workbenchScreen(){
  ...
  return `<div class="page">
  <div class="section-title">教室工作台 ${pill(stageLabel(c.stage), c.stage==='deployed'?'ok':'info')}</div>
  ...
  <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px">
    <button class="btn btn-primary" data-act="open-deployment">教室维护</button>
    ${!isBlank?`<button class="btn btn-secondary" data-act="open-maint-ip">修改IP/服务器地址</button>`:''}
  ...
  <table class="data-table">
    <thead><tr><th>#</th><th>当前工作 MAC</th>${isBlank?'':'<th data-sort>机位</th><th data-sort>机器名</th><th data-sort>IP</th>'}<th data-sort>状态</th></tr></thead>
    <tbody>${[...terms].map((t,i)=>{
      const isMother=t.id===m.id;
      return `<tr class="${!t.online?'offline-row':''}${isMother?' mother-row':''}">
      <td>${i+1}</td><td class="mono">${esc(t.mac)}${isMother?' '+pill('本机·母机','info'):''}</td>
      ${isBlank?'':`<td>${esc(t.seat||'--')}</td><td>${esc(t.name||'--')}</td><td class="mono">${esc(t.ip||'--')}</td>`}
...
function deployPrepScreen(){
  ...
  const checks = [];
  if(!m.serverAddr) checks.push('服务器地址未配置 — 请到首页"服务器连接"设置');
  if(!m.ip) checks.push('本机IP未配置 — 请到首页"服务器连接"设置');
  if(!desktops.length) checks.push('本机无桌面 — 请到首页"桌面管理"添加');
  if(!m.gateway) checks.push('网关未配置 — 请到首页"服务器连接"设置');
      ...
      <div style="display:flex;gap:16px;align-items:center;margin-bottom:12px;padding:10px 14px;background:var(--t-panel);border:1px solid var(--t-border);border-radius:6px;flex-wrap:wrap">
        ...
        <div style="display:flex;gap:4px">
          ${[['tl','↘ 左上'],['tr','↙ 右上'],['bl','↗ 左下'],['br','↖ 右下']].map(([v,l])=>
            `<button class="btn btn-sm ${dir===v?'btn-primary':'btn-ghost'}" data-rule-dir="${v}">${l}</button>`
          ).join('')}
        </div>
        <div style="font-size:.78rem;color:var(--t-text2);margin-left:8px">编号排布</div>
        <div style="display:flex;gap:4px">
          <button class="btn btn-sm ${flow==='col'?'btn-primary':'btn-ghost'}" data-rule-flow="col">列优先（纵向）</button>
          <button class="btn btn-sm ${flow==='row'?'btn-primary':'btn-ghost'}" data-rule-flow="row">行优先（横向）</button>
        </div>
      </div>
...
function deployBindScreen(){
  ...
  <div class="section-sub">
    ...
    <span style="float:right;display:flex;gap:6px">
      <button class="btn btn-ghost btn-sm" data-act="deploy-bind-next">模拟绑定（下一台）</button>
      <button class="btn btn-ghost btn-sm" data-act="deploy-bind-all">一键全绑定</button>
    </span>
      ...
      for(let ri=0;ri<grid.rows;ri++){
        for(let ci=0;ci<grid.cols;ci++){
          ...
          html+=`<div class="gb ${isBound?'gb-bound':isNext?'gb-next':'gb-waiting'}" data-bind-skip="${b.idx}" style="cursor:pointer" title="${seat} · ${isBound?'点击禁用并顺延后续绑定':'点击禁用此位置'}">
            ...
            ${isBound?`<div class="gb-mac">${esc(binding.mac.slice(-8))}</div>`
      ...
      <div style="display:grid;grid-template-rows:repeat(${crRows},auto);grid-template-columns:repeat(${crCols},1fr);grid-auto-flow:column;gap:8px">
        ${sorted.map(t=>{
          ...
          if(isMother) return `<div class="dt-card" style="padding:8px 10px;font-size:.78rem;opacity:.5;border-left:3px solid var(--t-warn)">
            ...
            <div class="mono" style="font-size:.72rem;color:var(--t-text3);margin:2px 0">${esc(t.mac.slice(-8))}</div>
...
function faultReplaceScreen(){
  ...
  return `<div class="page">
  <div class="section-title"><button class="btn btn-ghost btn-sm" data-act="go-home">←</button> 替换故障终端</div>
  <div class="section-sub">选择教室和要替换的终端，本机将继承该终端的配置与桌面。</div>
  ...
  <div style="display:grid;grid-template-columns:240px 1fr;gap:16px;align-items:start">
    ...
    <div class="card">
      <div class="card-header">选择终端 ${selectedCr?'— '+esc(selectedCr.name):''}</div>
      ${!selectedCr?'<div style="font-size:.82rem;color:var(--t-text3);padding:16px 0;text-align:center">← 请先选择教室</div>'
        :crTerms.length?`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px">
          ${crTerms.map(t=>`
            <div class="dt-card ${!t.online?'offline-look':''}" style="cursor:pointer;padding:10px;text-align:center;font-size:.82rem" data-fault-select="${t.id}">
              <div style="font-weight:600;margin-bottom:4px">${esc(t.seat||'--')}</div>
              <div style="font-size:.75rem;color:var(--t-text2)">${esc(t.name||'--')}</div>
              <div class="mono" style="font-size:.7rem;color:var(--t-text3)">${esc(t.mac.slice(-8))}</div>
              <div style="margin-top:4px">${t.online?pill('在线','ok'):pill('离线','err')}</div>
            </div>
          `).join('')}
...
function faultResetScreen(){
  ...
  <div class="card mb-16" style="width:100%;border-color:var(--t-warn);background:var(--t-warn-bg,rgba(255,180,0,0.08));margin-top:16px">
    <div style="font-size:.85rem;color:var(--t-warn)">⚠ 重置将丢弃本机所有未同步的数据，以服务器注册内容为准。此操作不可撤销。</div>
  </div>

  <div style="display:flex;gap:10px">
    <button class="btn btn-primary" ${serverOk?'':'disabled'} data-act="fault-reset-confirm-with-dialog">确认重置</button>
    <button class="btn btn-ghost" data-act="go-home">取消</button>
  </div>
  </div>
</div>`;
...
function exportScreen(){
  ...
  <table class="data-table" style="font-size:.8rem">
    <thead><tr><th>#</th><th>MAC</th><th data-sort>机位</th><th data-sort>机器名</th><th data-sort>IP</th><th>硬盘序列号</th></tr></thead>
...
function bindAll(){
  ...
  root.querySelectorAll('[data-fault-select]').forEach(el=>{
    el.addEventListener('click',()=>{
      ...
      showTermConfirm('确认替换','确定要替换 <b>'+label+'</b> 吗？<br>本机将继承该终端的全部配置与桌面。此操作不可撤销。',()=>act('fault-replace-confirm-direct',{terminalId:tid}));
    });
  });
  ...
  if(fileInput){
    fileInput.addEventListener('change',(e)=>{
      ...
      if(isPackage) act('create-desktop-from-file',{name:name||'导入桌面', os:'Windows 11 23H2', importType:'package'});
      else showCreateDesktopDialog(name||'新建桌面');
    });
  }
